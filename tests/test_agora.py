...
def test_post_and_get_message(agora_instance):
    ...
    message_id = agora_instance.post_message(
        from_agent="Manager_1", to_agent_role="Researcher", content=task_content
    )

    assert (
        message_id is not None
    ), "Posting a message should return a unique message ID."
...
def test_post_and_get_reply(agora_instance):
    ...
    reply_content = {
        "answer": "The 'D' in SOLID stands for Dependency Inversion Principle."
    }
    agora_instance.post_reply(
        message_id, from_agent="Researcher_B", result=reply_content
    )
